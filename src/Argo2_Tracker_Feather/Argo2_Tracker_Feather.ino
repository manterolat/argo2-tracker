 ...
 * https://ukhas.org.uk/
 *
 * Tracker program to run in Adafruit Feather M0 Adalogger for HAB use.
 * Transmits data including: Temperature, Pressure and Humidity.
...
void loop() {
		...
		if (rf95.recv(rxMsg, &len)) {
			...
			do {
				...
				if (strlen((char*)rxMsg) > 5 && strchr((char*)rxMsg, '*') != NULL) {
					...
					snprintf(checksumCalc, sizeof(checksumCalc), "%04X\0", calc_CRC16(rxMsg, strlen((char*)rxMsg) - 5));
	...
	if (currTimeSec % interval == 0	&& (currTimeSec != lastTimeSec || currTimeMin != lastTimeMin)) {
		...
		sprintf(satellites, "%d", gpsParser.satellites.value());
		...
		sprintf((char*)txMsg,
			"%s,%u,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%u",
			CALLSIGN,												// Callsign
			sentID,													// Sentence ID
			time,													// Time
			latitude,												// Latitude
	...
	/*if (buzzerOn) {
		if (buzzerState == true) {
			if (millis() - buzzLastOn > buzzPeriod) {
			...
			if (millis() - buzzLastOn > buzzInterval) {
