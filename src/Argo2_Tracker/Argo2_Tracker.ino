 ...
 * https://ukhas.org.uk/
 *
 * Tracker program to run on a Teensy 3.1/3.2 or Adafruit Feather M0 for HAB use.
...
void setup() {
    ...
    /*** Setup GPS, BME280 and RFM96W ***/

    // Set U-Blox Nav Mode to 'Airborne < 1G' and Power Mode to 'Max Power' (retry until correct ACK received)
...
// Manage state/status, GPS updates, reception and transmission - repeat forever
void loop() {
    ...
    //      BUZZER:     OFF
    //
    //  1. Rising (vertical speed > 2.0 m/s and altitude > 500 m)
    //      TX_POWER:   23
    //      GPS_MODE:   Airborne
    //      GPS_POWER:  MAX
    //      BUZZER:     OFF
    //
    //  2. Falling (vertical speed < -2.0)
    //      TX_POWER:   23
    //      GPS_MODE:   Airborne
    //      GPS_POWER:  MAX
    //      BUZZER:     OFF
    //
    //  3. Falling, Low ALitude (vertical speed < -2.0 and altitude < 9,000 m)
    //      TX_POWER:   23
    //      GPS_MODE:   Pedestrian
    //      GPS_POWER:  MAX
    //      BUZZER:     OFF
    ...
    switch (state) {
        ...
        case STATE_STANDBY:
            ...
            // If vertical speed is greater than 2 m/s and altitude is greater than 500 m, switch to RISING state
            if (verticalSpeed > 2.0 && gpsParser.altitude.meters() > 500.0) {
                state = STATE_RISING;
        ...
        case STATE_RISING:
            ...
            // If vertical speed is less than -2 m/s, switch to FALLING_HIGH state.
        ...
        case STATE_FALLING_HIGH:
            ...
            // If vertical speed is less than -2 m/s and altitude is less than 9,000 m, switch to FALLING_LOW state.
            if (verticalSpeed < -2.0 && gpsParser.altitude.meters() < 9000.0) {
    ...
    if (currTimeSec % interval == 0 && (currTimeSec != lastTimeSec || currTimeMin != lastTimeMin)) {
        ...
        // If using MAX31855 thermocouple, use it to get Temperature
        #ifdef MAX31855
        ...
        // Otherwise use BME280 (less accurate at low temperatures)
        #else
            double extTemperature = bme.readTemperature();          // External Temperature in °C
            double intTemperature = 0;                              // Internal Temperature in °C
        #endif
        ...
        // Feather M0 and AVR can't use 'sprintf' for floating-point to string convertions.
        // Must use 'dtostrf' instead.
        // Teensy can use 'sprintf'
        #ifdef TEENSY
        ...
        // Use 'dtostrf' for Feather M0 and AVR. Teensy should just use 'sprintf'
        #ifdef TEENSY
